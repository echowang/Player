...
public class ChineseToPinyin {
    ...
    private static ChineseToPinyin sInstance;
    ...
    private static class Token {
        ...
        private static final int PINYIN = 2;
        ...
        private Token(int type, String source, String target) {
            this.type = type;
            this.source = source;
            this.target = target;
        }

        public boolean equalsAsPinYin(Token tokenCompare) {
            if (type != tokenCompare.type)
                return false;

            if (type == PINYIN)
                return target.equals(tokenCompare.target);

            return source.equals(tokenCompare.source);
        }

        public boolean equalsAsPinYinFuzzy(Token tokenCompare) {
            if (type != tokenCompare.type)
                return false;
            if (type == PINYIN && target != null && tokenCompare.target != null) {
                String pinyinThis = target.toUpperCase();
                String pinyinCompare = tokenCompare.target.toUpperCase();
                //处理卷舌音
                pinyinThis = pinyinThis.replace("CH", "C");
                pinyinThis = pinyinThis.replace("SH", "S");
                pinyinThis = pinyinThis.replace("ZH", "H");
                pinyinCompare = pinyinCompare.replace("CH", "C");
                pinyinCompare = pinyinCompare.replace("SH", "S");
                pinyinCompare = pinyinCompare.replace("ZH", "H");
                //处理前后鼻音
                pinyinThis = pinyinThis.replace("ANG", "AN");
                pinyinThis = pinyinThis.replace("ENG", "EN");
                pinyinThis = pinyinThis.replace("ING", "IN");
                pinyinThis = pinyinThis.replace("ONG", "ON");
                pinyinCompare = pinyinCompare.replace("ANG", "AN");
                pinyinCompare = pinyinCompare.replace("ENG", "EN");
                pinyinCompare = pinyinCompare.replace("ING", "IN");
                pinyinCompare = pinyinCompare.replace("ONG", "ON");
                //处理鼻音和边音
                if (!pinyinThis.isEmpty()) {
                    if (pinyinThis.charAt(0) == 'N') {
                        pinyinThis = "L" + pinyinThis.substring(1);
                    }
                }
                if (!pinyinCompare.isEmpty()) {
                    if (pinyinCompare.charAt(0) == 'N') {
                        pinyinCompare = "L" + pinyinCompare.substring(1);
                    }
                }
                return pinyinThis.equals(pinyinCompare);
            }
            return source.equals(tokenCompare.source);
        }
    ...
    private ChineseToPinyin(boolean hasChinaCollator) {
        mHasChinaCollator = hasChinaCollator;
    }

    public static ChineseToPinyin getInstance() {
        synchronized (ChineseToPinyin.class) {
            if (sInstance != null) {
                return sInstance;
            }
            if (COLLATOR != null) {
                sInstance = new ChineseToPinyin(true);
            } else {
                sInstance = new ChineseToPinyin(false);
            }
            return sInstance;
        }
    }
    ...
    public static String getPinYin(String input) {
        ArrayList<Token> tokens = ChineseToPinyin.getInstance().get(input);
